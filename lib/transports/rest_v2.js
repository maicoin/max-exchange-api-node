...
class RestV2 {
   ...
   */
  profile () {
    return this._request.getPrivate('/api/v2/members/profile')
  }
   ...
   */
  accounts () {
    return this._request.getPrivate('/api/v2/members/accounts')
  }

  /**
   * @param {string} - unique currency id, check currencies() for available currencies
   *
   * @return {Promise}
   *
   ...
   */
  account (currency) {
    return this._request.getPrivate(`/api/v2/members/accounts/${currency}`)
  }
  ...
  /**
   * @param {Object} options
   * @param {string} options.amount - withdraw amount in TWD
   *
   * @return {Promise}
   *
   * @see https://max.maicoin.com/documents/api_list/v2#!/private/postApiV2WithdrawalTwd
   */
  createTwdWithdrawal (options) {
    return this._request.postPrivate('/api/v2/withdrawal/twd', options, optionsSchema.createTwdWithdrawal)
  }

  /**
   * @param {Object} options
   * @param {string} options.currency - unique currency id
   * @param {string} options.withdraw_address_uuid - unique withdraw address id
   * @param {string} options.amount - withdraw amount
   *
   * @return {Promise}
   *
   * @see https://max.maicoin.com/documents/api_list/v2#!/private/postApiV2Withdrawal
   */
  createCryptoWithdrawal (options) {
    return this._request.postPrivate('/api/v2/withdrawal', options, optionsSchema.createCryptoWithdrawal)
  }
   ...
   */
  withdrawAddresses (options) {
    return this._request.getPrivate('/api/v2/withdraw_addresses', options, optionsSchema.withdrawAddresses)
  }
  ...
  /**
   * @param {Object} options
   * @param {string} options.side - 'in' or 'out', default as 'in'
   * @param {string} [options.currency] - unique currency id
   * @param {number} [options.from] - target period start (Epoch time in seconds)
   * @param {number} [options.to] - target period end (Epoch time in seconds)
   * @param {number} [options.limit=50] - returned results limit, maximum 1000
   * @param {number} [options.page=1] - specify the page of paginated results
   *
   * @return {Promise}
   *
   ...
   */
  internalTransfers (options) {
    return this._request.getPrivate('/api/v2/internal_transfers', options, optionsSchema.internalTransfers)
  }
}
