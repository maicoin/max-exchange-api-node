import type {
  ...
  Reward, Ticker, Timestamp, Trade, UserInfo, Withdrawal, Depth, Repayment,
  ForcedLiquidation,
  VipLevel,
  CurrencyNetwork,
  IndexPrices,
...
export const convertToCurrency = (data: any): Currency => {
  return {
    ...
    networks: data.networks.map((d) => {
      return convertToCurrencyNetwork(d)
    })
  };
}
...
export const convertToInterestRate = (data: Record<string, { hourlyInterestRate: string, nextHourlyInterestRate: string }>): BorrowingInterestRates => {
...
export const convertToKLine = (data: [number, string, string, string, string, string][]): [Date, Decimal, Decimal, Decimal, Decimal, Decimal][] => {
